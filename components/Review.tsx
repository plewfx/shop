...
const generateStars = (starCount: number) => {
  ...
  let stars = Array.from({length: Math.floor(starCount)}, () => <Image src="/star.svg" width={22.5} height={22.5} alt="star" />);

  Number.isInteger(starCount) || stars.push(<Image src="/half-star.svg" width={10} height={22.5} alt="star" />);
...
const Review = ({ review }: { review: ReviewType }) => {
  return (
    <div className="flex flex-col gap-4 sm:gap-3 py-7 px-8 sm:p-6 rounded-[20px] border border-[rgba(0,0,0,0.10)]">
        ...
        <div className="flex flex-col gap-4 sm:gap-2">
            ...
            <q className="text-base text-[rgba(0,0,0,0.60)]">{review.text}</q>
        </div>
    </div>
  )
}
